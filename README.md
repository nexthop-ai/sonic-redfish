...
- `bmcweb/subprojects/sdbusplus.wrap` - D-Bus C++ bindings (revision pinned by
  `0003-pin-sdbusplus-to-known-good-revision.patch`)
- `bmcweb/subprojects/stdexec.wrap` - C++23 executors (revision pinned via
  `STDEXEC_REVISION`, since upstream defaults to `HEAD`)
